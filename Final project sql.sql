...
from customer_churn_dataset group by churn_status;
select count(churn_status) from customer_churn_dataset where churn_status = 'yes'
...
from customer_churn_dataset group by contract_type,churn_status;
...
select customer_id,total_charges
from customer_churn_dataset where churn_status= 'no';
...
from customer_churn_dataset where churn_status ='yes' group by age_group;
...
-- 17.	Calculate the average age and total charges for customers with different combinations of streaming services
 ...
 from customer_churn_dataset group by streaming_tv,streaming_movies;
...
from customer_churn_dataset where churn_status ='yes'and contract_type = 'yearly'group by gender;
...
from customer_churn_dataset where churn_status = 'yes' group by contract_type,internet_service;
...
from customer_churn_dataset where churn_status = 'yes' and online_backup = 'no' and online_security = 'no' 
...
from customer_churn_dataset where churn_status = 'yes'
group by dependents;

-- 22.	Identify the customers who have churned, and their contract duration in months (for monthly contracts) ---
select customer_id,contract_type from customer_churn_dataset
...
create view bhuvana as
select max(monthly_charges) as highest_monthly_charges,contract_type
from customer_churn_dataset group by contract_type;
select * from bhuvana;
...
select avg(monthly_charges),'total' as entire_customers from customer_churn_dataset
union
select avg(monthly_charges),'churned' as entire_customers from customer_churn_dataset where churn_status ='yes';
...
create view bhuvana_2 as
  select customer_id,sum(total_charges) from customer_churn_dataset
  group by customer_id;
...
select count(churn_status),(churn_status) from customer_churn_dataset
...
from customer_churn_dataset where churn_status = 'no'  and 
